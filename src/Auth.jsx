  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }} onClick={onClose}>
      <div style={{ background: "#111", borderRadius: "16px", width: "100%", maxWidth: "460px", border: "1px solid #1e1e1e", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        {/* Mode toggle */}
        <div style={{ display: "flex", background: "#1a1a1a", margin: "20px 24px 0", borderRadius: "10px", padding: "4px" }}>
          {["signin", "signup"].map(m => (
            <button
              key={m}
              onClick={() => { setMode(m); setErrors({}); setServerError(""); setShowPassword(false); setShowConfirmPassword(false) }}
              style={{ flex: 1, padding: "10px", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "700", fontSize: "13px", fontFamily: "inherit", background: mode === m ? "#c8a97e" : "transparent", color: mode === m ? "#000" : "#666", transition: "all .2s" }}>
        ...
        <div style={{ padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          {mode === "signin" && (
            <div style={{ textAlign: "center", fontSize: "13px", color: "#555" }}>
              Forgot your password? <span style={{ color: "#c8a97e", cursor: "pointer" }}>Reset it</span>
            </div>
          )}

          <div style={{ textAlign: "center", fontSize: "12px", color: "#444", marginTop: "4px" }}>
