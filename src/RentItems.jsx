          ...
          <div style={{ background: "#161616", borderRadius: "14px", padding: "20px", border: "1px solid #1e1e1e", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            {!rated ? (
              <>
                ...
                <button className="btn-gold" onClick={handleRate} disabled={!rating} style={{ padding: "12px", borderRadius: "10px", fontSize: "14px", opacity: rating > 0 ? 1 : 0.4, cursor: rating > 0 ? "pointer" : "not-allowed" }}>
                  Submit Rating
                </button>
              </>
            ) : (
...
function RentalTimer({ endTime }) {
  ...
  useEffect(() => {
    const i = setInterval(() => setTimeLeft(endTime - Date.now()), 1000)
  ...
  return (
    <div style={{ background: exp ? "#7f1d1d18" : near ? "#78350f18" : "#064e3b18", border: `1px solid ${exp ? "#7f1d1d" : near ? "#92400e" : "#065f46"}`, borderRadius: "14px", padding: "18px", textAlign: "center" }}>
      <div style={{ fontSize: "11px", color: exp ? "#fca5a5" : near ? "#fcd34d" : "#6ee7b7", fontWeight: "700", marginBottom: "10px", textTransform: "uppercase", letterSpacing: ".08em" }}>
      ...
      {!exp && (
        <div style={{ display: "flex", justifyContent: "center", gap: "16px" }}>
          {[["Days", days], ["Hours", hours], ["Mins", mins], ["Secs", secs]].map(([label, val]) => (
            <div key={label}>
              <div style={{ fontSize: "28px", fontWeight: "800", color: near ? "#fcd34d" : "#c8a97e", lineHeight: 1, letterSpacing: "-0.02em" }}>{String(val).padStart(2, "0")}</div>
...
export default function RentItems({ rate, siteSettings }) {
  ...
  const depositAmt   = selected?.accountability ? Math.round(getDailyRate(selected) * (selected.accountabilityPct / 100)) : 0
  const totalCost    = rentalCost + depositAmt
  ...
  const openRental = (item) => {
    ...
    setPayLoading(false); setRentalTimerEnd(null)
    setRenterConfirmed(false); setCancelled(false); setShowRating(false)
  }
  ...
  const handlePay = async () => {
    ...
    const order = {
      id: orderId, type: "rent",
      cart: [{
        ...selected,
        title: selected.title,
        price: getDailyRate(selected),
        qty: days,
        seller: selected.seller || { _id: null, name: getLenderName(selected) },
        image: getImg(selected),
      }],
      item: selected, days, rentalCost, depositAmount: depositAmt,
      totalCost, total: totalCost, subtotal: rentalCost,
      platformFee, lenderGets, contactInfo,
      deliveryMethod: "pickup",
      paymentMethod: "manual_momo",
      paymentRef: `RENT-${orderId}`,
      status: "Pending Confirmation",
      renterConfirmed: false, lenderConfirmed: false,
          ...
          {loadingListings ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "20px" }}>
              {filtered.map((item, index) => (
                <div key={item._id || item.id} className="listing-card" style={{ animationDelay: `${(index % 8) * 55}ms` }}>
                  ...
                  <div style={{ padding: "16px" }}>
                    ...
                    <div style={{ fontSize: "11px", color: "#333", marginBottom: "14px" }}>🎓 {getUniversity(item)} · {item.condition || "Good"}</div>
                    ...
                    <div style={{ fontSize: "20px", fontWeight: "800", color: "#c8a97e", marginBottom: "14px", letterSpacing: "-0.02em" }}>
                      ...
                      <span style={{ fontSize: "12px", color: "#333", fontWeight: "400" }}> (${toUSD(getDailyRate(item))})</span>
                    </div>
        ...
        <div className="modal-backdrop" style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }} onClick={() => setDetailItem(null)}>
          <div className="modal-content" style={{ background: "#111", borderRadius: "20px", width: "100%", maxWidth: "500px", maxHeight: "90vh", overflowY: "auto", border: "1px solid #1e1e1e" }} onClick={e => e.stopPropagation()}>
            <div style={{ position: "relative" }}>
              ...
              <button onClick={() => setDetailItem(null)} style={{ position: "absolute", top: "14px", right: "14px", background: "#000000aa", border: "none", color: "#fff", fontSize: "18px", cursor: "pointer", width: "34px", height: "34px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(8px)" }}>✕</button>
            </div>
            <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
              <div>
                ...
                <div style={{ fontSize: "13px", color: "#555" }}>Listed by <span style={{ color: "#888", fontWeight: "600" }}>{getLenderName(detailItem)}</span></div>
                <div style={{ fontSize: "12px", color: "#444", marginTop: "3px" }}>🎓 {getUniversity(detailItem)}</div>
              </div>
              {detailItem.rating > 0 && (
                <div style={{ fontSize: "13px", color: "#c8a97e" }}>{"★".repeat(Math.round(detailItem.rating))}{"☆".repeat(5 - Math.round(detailItem.rating))} <span style={{ color: "#555" }}>{detailItem.rating} ({detailItem.reviews || 0} reviews)</span></div>
              )}
              ...
              <div style={{ background: "#1a1a1a", borderRadius: "12px", padding: "16px", fontSize: "13px", display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                {detailItem.accountability && <div>🔒 Deposit: <span style={{ color: "#fcd34d", fontWeight: "700" }}>{detailItem.accountabilityPct}% of daily rate</span></div>}
              </div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid #1a1a1a", paddingTop: "18px" }}>
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: "#444" }}>${toUSD(getDailyRate(detailItem))}/day USD</div>
                </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "18px", justifyContent: "center" }}>
                      <button onClick={() => setDays(d => Math.max(1, d - 1))} style={{ width: "44px", height: "44px", borderRadius: "50%", background: "#161616", border: "1px solid #222", color: "#fff", fontSize: "20px", cursor: "pointer", minHeight: "auto" }}>−</button>
                      <span style={{ fontSize: "36px", fontWeight: "800", color: "#c8a97e", minWidth: "60px", textAlign: "center", letterSpacing: "-0.03em" }}>{days}</span>
                      <button onClick={() => setDays(d => Math.min(selected.maxDays || 7, d + 1))} style={{ width: "44px", height: "44px", borderRadius: "50%", background: "#161616", border: "1px solid #222", color: "#fff", fontSize: "20px", cursor: "pointer", minHeight: "auto" }}>+</button>
                    </div>
                  ...
                  <div style={{ background: "#161616", borderRadius: "14px", padding: "18px", display: "flex", flexDirection: "column", gap: "10px" }}>
                    ...
                    {depositAmt > 0 && (
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#666" }}>
                        <span>Accountability deposit ({selected.accountabilityPct}%)</span><span>₵{depositAmt}</span>
                      </div>
                    )}
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#666" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "19px", fontWeight: "800", color: "#c8a97e", borderTop: "1px solid #222", paddingTop: "10px", letterSpacing: "-0.02em" }}>
                      <span>Total</span><span>₵{totalCost} (${toUSD(totalCost)})</span>
                    </div>
                    {depositAmt > 0 && (
                      <div style={{ fontSize: "12px", color: "#6ee7b7", background: "#064e3b18", borderRadius: "10px", padding: "10px 14px" }}>
                        🔒 Deposit of ₵{depositAmt} fully refunded if returned undamaged.
                      </div>
                    )}
                  </div>

                  <div>
                    <div style={{ fontSize: "11px", color: "#444", fontWeight: "700", marginBottom: "8px", textTransform: "uppercase", letterSpacing: ".08em" }}>YOUR CONTACT (for the lender)</div>
              ...
              {step === 2 && (
                <>
                  <div style={{ background: "#ffd700", borderRadius: "14px", padding: "20px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: "13px", color: "#554400", marginTop: "4px" }}>${toUSD(totalCost)} USD</div>
                  </div>
                  <div style={{ background: "#161616", borderRadius: "14px", padding: "18px", fontSize: "13px", color: "#888", display: "flex", flexDirection: "column", gap: "8px" }}>
                    ...
                    {depositAmt > 0 && <div>🔒 Deposit: <span style={{ color: "#fcd34d" }}>₵{depositAmt} held until return</span></div>}
                  ...
                  <div style={{ background: "#78350f18", border: "1px solid #92400e", borderRadius: "12px", padding: "14px", fontSize: "13px", color: "#fcd34d", display: "flex", flexDirection: "column", gap: "6px" }}>
                    ...
                    <div>3. Amount: <strong>₵{totalCost}</strong> · Ref: <strong style={{ fontFamily: "monospace" }}>{orderId}</strong></div>
              ...
              {step === 3 && !cancelled && (
                <>
                  ...
                  <div style={{ background: "#161616", borderRadius: "14px", padding: "16px", fontSize: "13px", display: "flex", flexDirection: "column", gap: "8px" }}>
                    ...
                    <div>💰 Total paid: <span style={{ color: "#888" }}>₵{totalCost}</span></div>
                    {depositAmt > 0 && <div>🔒 Deposit: <span style={{ color: "#fcd34d" }}>₵{depositAmt} (refunded if undamaged)</span></div>}
                  </div>
              ...
              {step === 4 && !showRating && rentalTimerEnd && (
                <>
                  ...
                  <div style={{ background: "#161616", borderRadius: "14px", padding: "16px", fontSize: "13px", color: "#666" }}>
                    ...
                    {depositAmt > 0 && <div style={{ marginTop: "8px" }}>🔒 Deposit held: <span style={{ color: "#fcd34d" }}>₵{depositAmt}</span> — returned if no damage</div>}
                  </div>
                  {!renterConfirmed ? (
              ...
              {step === 4 && showRating && (
                <>
                  <div style={{ textAlign: "center" }}>
                    ...
                    {depositAmt > 0 && <p style={{ fontSize: "13px", color: "#888" }}>₵{depositAmt} deposit returned once lender confirms no damage.</p>}
                  </div>
